...
export default function TeacherForgotPassword() {
  ...
  const handleOtpVerify = async e => {
    ...
    setSuccess('');
    try {
      const res = await api.post('/teacher/verify-otp', { email, otp });
      setSuccess(res.data.message || 'OTP verified!');
      setTimeout(() => navigate('/reset-password', { state: { email, role: 'teacher' } }), 1000);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-50 to-gray-100">
      <form onSubmit={otpSent ? handleOtpVerify : handleSubmit} className="bg-white shadow-lg rounded-xl p-8 w-full max-w-md">
        <h2 className="text-2xl font-bold text-orange mb-6 text-center">Teacher Forgot Password</h2>
        <div className="mb-4">
